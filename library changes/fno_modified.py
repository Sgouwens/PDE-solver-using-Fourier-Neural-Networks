...
class ParameterBlock(nn.Module):
    def __init__(self, image_channels: int, input_param_dim: int, hidden_param_dim: int):
        super().__init__()
        
        self.parameter_net = nn.Sequential(
            nn.Linear(input_param_dim, 4*hidden_param_dim),
            nn.GELU(),
            nn.Linear(4*hidden_param_dim, hidden_param_dim)
        )
        # The input to fc is the concatenated tensor: image_channels + param_dim
        self.fc = nn.Linear(image_channels + hidden_param_dim, 
                            image_channels + hidden_param_dim)
        self.gelu = nn.GELU()
        #self.output_conv = nn.Conv2d(in_channels=hidden_param_dim, out_channels=hidden_param_dim, kernel_size=1)

    def forward(self, image: torch.tensor, parameters: List):
        # Create a new tensor with the parameters expanded to match the image dimensions of [batch_size, param_dim]
        # Then the expanded parameters are concatenated to the original image along the channels

        param_features = self.parameter_net(parameters) # [B, 1] -> [B, hidden_param_dim]
        param_features_expanded = (param_features       # [B, hidden_param_dim] -> [B, hidden_param_dim, 64, 64]
                                   .unsqueeze(-1).unsqueeze(-1).expand(-1, -1, image.size(2), image.size(3)))
        
        # print("param_features_expanded shape is: ", param_features_expanded.shape)
        image_with_parameters = torch.cat([image, param_features_expanded], dim=1) # [B, 1, 64, 64] -> [B, 1+hidden_param_dim, 64, 64]
        
        # print("image_with_parameters shape is: ", image_with_parameters.shape)
        # Move channels last for the Linear layer (note that Linear() applies to the last dimension)
        combined = image_with_parameters.permute(0, 2, 3, 1) # [B, C, H, W] -> [B, H, W, C]
        combined = self.fc(combined) # [N, H, W, hidden_param_dim]
        combined = self.gelu(combined)
        
        # Undo the permute so the formatting of the data is the same after Linear().This comes with almost NO computational costs!
        combined = combined.permute(0, 3, 1, 2) # [B, H, W, C] -> [B, C, H, W]

        # print("combined shape is: ", combined.shape, "compared to the input image shape of",  image.shape)
        
        return combined # [N, hidden_param_dim, H, W]
### Einde toevoeging Sigur


class FNO_modified(BaseModel, name='FNO'):
    ...
    def __init__(
        ...
        input_param_dim: int,    # Added by Sigur
        hidden_param_dim: int,   # Added by Sigur
        ...
        **kwargs
        ...
        self.input_param_dim = input_param_dim # Added by Sigur

        self.hidden_param_dim = hidden_param_dim # Added by Sigur
        ...
        ### Added by Sigur. Idea: We add a small extra network that handles additional parameters. 
        # The dual-network will be a simple fully connected MLP to incorporate the sensitivity
        # parameters in a non-linear and possibly interactive way. The output will be a tensor of 
        # dimensions d_1 x d_2 x H where d_i are the dimensions of the simulation of interest, and
        # H is the parametric dimension. For each channel h in H, the image values are constant 
        # and then concatenated to the FNO network.
        self.parameter_block = ParameterBlock(image_channels=in_channels, input_param_dim=input_param_dim, hidden_param_dim=hidden_param_dim)
        ### End addition by Sigur
        ...
        ### Sigur changed this line: lifting_in_channels = self.in_channels
        lifting_in_channels = self.in_channels + self.hidden_param_dim#lifting_in_channels = self.hidden_channels 
        ### End change Sigur
    ...
    ### Sigur changed this line
    def forward(self, x, parameters, output_shape=None, **kwargs): # def forward(self, x, output_shape=None, **kwargs):
        ### End change Sigur
        """FNO's forward pass
        ...
        ### Sigur changed this.
        x = self.parameter_block(x, parameters)
        ###
...
#TFNO_modified = partialclass("TFNO", FNO_modified, factorization="Tucker")